...
def create_pydantic_ai_coder(custom_model: Any | None = None):
    ...
    # In unit tests we may pass a placeholder string like "dummy-model".
    # If we're using the real pydantic_ai.Agent, map it to the built-in "test" model
    # to avoid provider resolution. If Agent is monkeypatched (FakeAgent), keep the
    # original value so tests can assert it verbatim.
    if isinstance(selected_model, str) and selected_model == "dummy-model":
        agent_module = getattr(Agent, "__module__", "")
        if agent_module.startswith("pydantic_ai"):
            selected_model = "test"
